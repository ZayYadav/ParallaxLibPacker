...
void PeFile::checkHeaderValues(unsigned subsystem, unsigned mask, unsigned ih_entry,
    ...
    if (memcmp(isection[0].name, "PARA", 4) == 0)
...
void PeFile::pack0(OutputFile *fo, ht &ih, ht &oh, unsigned subsystem_mask,
    ...
    // fill the sections
    strcpy(osection[0].name, "PARA0");
    strcpy(osection[1].name, "PARA1");
    ...
    // locate version info
    strcpy(osection[2].name, !last_section_rsrc_only && soresources ? ".rsrc" : "PARA2");
