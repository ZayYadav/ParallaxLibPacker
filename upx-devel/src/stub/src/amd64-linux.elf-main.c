...
// Custom Security Sycall Wrappers
static long upx_ptrace(long request, long pid, void *addr, void *data) {
    long res;
    asm volatile("mov $101, %%rax; syscall" : "=a"(res) : "D"(request), "S"(pid), "d"(addr), "r"(data) : "rcx", "r11", "memory");
    return res;
}

static long upx_prctl(int option, unsigned long arg2, unsigned long arg3, unsigned long arg4, unsigned long arg5) {
    long res;
    asm volatile("mov $157, %%rax; syscall" : "=a"(res) : "D"(option), "S"(arg2), "d"(arg3), "r"(arg4), "r"(arg5) : "rcx", "r11", "memory");
    return res;
}

static long upx_open(const char *pathname, int flags, int mode) {
    long res;
    asm volatile("mov $2, %%rax; syscall" : "=a"(res) : "D"(pathname), "S"(flags), "d"(mode) : "rcx", "r11", "memory");
    return res;
}

static long upx_read(int fd, void *buf, size_t count) {
    long res;
    asm volatile("mov $0, %%rax; syscall" : "=a"(res) : "D"(fd), "S"(buf), "d"(count) : "rcx", "r11", "memory");
    return res;
}

static long upx_close(int fd) {
    long res;
    asm volatile("mov $3, %%rax; syscall" : "=a"(res) : "D"(fd) : "rcx", "r11", "memory");
    return res;
}

static int upx_strstr(const char *haystack, const char *needle) {
    if (!*needle) return 1;
...
static void security_check() {
    ...
    // 1. Anti-Debug: ptrace traceme
    if (upx_ptrace(0, 0, (void *)1, 0) < 0) {
        *(int*)0 = 0xDEAD; // CRASH
    }

    // 2. Anti-Frida / Anti-Hook: Scan /proc/self/maps
    fd = upx_open("/proc/self/maps", 0, 0); // O_RDONLY
    if (fd >= 0) {
        while (upx_read(fd, buf, sizeof(buf) - 1) > 0) {
            buf[sizeof(buf)-1] = 0;
            if (upx_strstr(buf, "frida") || upx_strstr(buf, "gum-js") || upx_strstr(buf, "substitute")) {
                upx_close(fd);
    ...
    // 3. VPN / Network Proxy Detection (Check for tun/tap interfaces in /proc/net/dev)
    fd = upx_open("/proc/net/dev", 0, 0);
    if (fd >= 0) {
        while (upx_read(fd, buf, sizeof(buf) - 1) > 0) {
            buf[sizeof(buf)-1] = 0;
            if (upx_strstr(buf, "tun0") || upx_strstr(buf, "ppp0") || upx_strstr(buf, "tap0")) {
                upx_close(fd);
                *(int*)0 = 0xDEADC0DE; // CRASH
            }
        }
        upx_close(fd);
    }

    // 4. Anti-Debug: prctl set ptracer to 0
    upx_prctl(0x59616d61, 0, 0, 0, 0); // PR_SET_PTRACER, 0
}
