...
#endif
    // clang-format on
    fprintf(f, "Parallax comes with ABSOLUTELY NO WARRANTY; for details type '%s -L'.\n", progname);
}
...
void show_sysinfo(const char *options_var) {
    ...
    if (opt->verbose >= 1) {
        con_fprintf(f, "Parallax version: ");
        fflush(f);
        show_version(true);
        con_fprintf(f, "Parallax version internal: 0x%06x %s\n", UPX_VERSION_HEX, UPX_VERSION_STRING);
    }
    fflush(stdout);
